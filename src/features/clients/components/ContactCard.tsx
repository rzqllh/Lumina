...
export const ContactCard: React.FC<ContactCardProps> = ({ contact, onEdit, onDelete }) => {
  return (
    <div
      ...
      className="flex flex-col justify-between rounded-xl border border-border bg-surface p-4 shadow-2xs transition-all sm:flex-row sm:items-center"
    >
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-sm font-bold text-text-primary">{contact.name}</span>
          {contact.is_primary && (
            <span className="inline-flex items-center gap-1 rounded-md bg-purple-50 px-2 py-0.5 text-[10px] font-semibold text-primary border border-purple-200">
              <Star className="h-2.5 w-2.5 fill-primary" /> Primary
            </span>
          )}
          {contact.role_label && (
            <span className="rounded-md bg-surface-muted px-2 py-0.5 text-[10px] font-medium text-text-secondary border border-border-subtle">
              {contact.role_label}
            </span>
          )}
        </div>
      ...
      <div className="mt-3 flex items-center justify-end gap-1.5 sm:mt-0">
        ...
        <button
          ...
          className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border border-border bg-surface text-text-muted transition-colors hover:bg-status-danger/10 hover:border-status-danger/30 hover:text-status-danger focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
