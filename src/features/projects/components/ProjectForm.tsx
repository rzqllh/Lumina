    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <label htmlFor="client_id" className="block text-xs font-semibold text-text-primary">
            Client <span className="text-status-danger">*</span>
          </label>
          <Link to="/clients/new" className="text-[11px] font-medium text-primary hover:underline">
            + Add New Client
          </Link>
        </div>
        ...
        {isClientsLoading ? (
          ...
          <select
            ...
            {...register('client_id')}
            className={`w-full rounded-xl border bg-surface px-3.5 py-2.5 text-sm text-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring transition-colors ${
              errors.client_id ? 'border-status-danger' : 'border-border'
            }`}
          >
        ...
        {errors.client_id && (
          <p className="mt-1 text-xs text-status-danger">{errors.client_id.message}</p>
        )}
      ...
      {/* Project Title */}
      <div>
        <label htmlFor="title" className="block text-xs font-semibold text-text-primary mb-1.5">
          Project Title <span className="text-status-danger">*</span>
        </label>
        <input
          id="title"
          type="text"
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
          <label htmlFor="status" className="block text-xs font-semibold text-text-primary mb-1.5">
            System Status
          </label>
          <select
            id="status"
