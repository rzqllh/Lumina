...
export function PackagePickerModal({ onApply, onClose, isApplying }: PackagePickerModalProps) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-2xl border border-border bg-surface shadow-xl flex flex-col max-h-[80vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
          <div className="flex items-center gap-2">
            <Package className="h-4 w-4 text-primary" aria-hidden="true" />
            <h2 className="text-sm font-bold text-text-primary">Apply Package</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {error && (
            <div role="alert" className="flex items-center gap-2 p-4 text-xs text-status-danger">
              ...
              {activePackages.map((pkg) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left cursor-pointer rounded-xl border px-4 py-3 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
                      isSelected
                        ? 'border-primary bg-purple-50/60 ring-1 ring-primary/30'
