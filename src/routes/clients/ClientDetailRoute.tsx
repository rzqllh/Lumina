...
export function ClientDetailRoute() {
  ...
  if (error || !client) {
    return (
      ...
      >
        ...
        <h3 className="text-sm font-bold text-text-primary">Client not found</h3>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              {client.is_archived && (
                <span className="inline-flex items-center rounded-md bg-zinc-100 px-1.5 py-0.5 text-[10px] font-medium text-zinc-600 border border-zinc-200">
                  Archived
                </span>
              )}
            </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="inline-flex cursor-pointer items-center gap-1.5 rounded-xl bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          >
        ...
        {client.contacts.length === 0 ? (
          <div
            data-testid="empty-contacts-state"
            className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-surface/50 p-6 text-center"
          >
            <p className="text-xs text-text-muted">
              No person contacts added yet. Add contacts like Bride, Groom, or Corporate PIC.
            </p>
          </div>
        ) : (
      ...
      {/* Projects Association Section (Placeholder for Feature #3) */}
      <div className="space-y-3 pt-2">
        <div className="flex items-center gap-2">
          <FolderKanban className="h-4 w-4 text-text-muted" />
          <h2 className="text-sm font-bold text-text-primary">Projects (0)</h2>
        </div>
        <div className="rounded-xl border border-border bg-surface p-5 text-center text-xs text-text-muted shadow-2xs">
